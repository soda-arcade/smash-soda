...
public:
	...
	bool run() override
	{
		...
		{
			_replyMessage = std::string() + MetadataCache::preferences.chatbotName + " | Usage: !speakers <integer in range [0, 100]>\nExample: !speakers 42\0";
			return false;
		}
		...
		reply << MetadataCache::preferences.chatbotName + " | Speakers volume set to " << _intArg << "%\0";
