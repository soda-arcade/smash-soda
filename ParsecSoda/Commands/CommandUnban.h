...
public:
	...
	bool run() override
	{
		...
		{
			_replyMessage = MetadataCache::preferences.chatbotName + " | Usage: !unban <username>\nExample: !unban melon\0";
			return false;
		}
		...
		if (found)
		{
			...
			reply
				<< MetadataCache::preferences.chatbotName + " | " << _sender.name << " has revoked a ban:\n"
		...
		{
			_replyMessage = std::string() + MetadataCache::preferences.chatbotName + " | " + _sender.name + ", I cannot find the user you want to unban.\0";
			return false;
		}
	}
