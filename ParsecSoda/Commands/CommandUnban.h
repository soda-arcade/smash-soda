...
public:
	...
	bool run() override
	{
		...
		{
			_replyMessage = "[MasterHand] | Usage: !unban <username>\nExample: !unban melon\0";
			return false;
		}
		...
		if (found)
		{
			...
			reply
				<< "[MasterHand] | " << _sender.name << " has revoked a ban:\n"
		...
		{
			_replyMessage = std::string() + "[MasterHand] | " + _sender.name + ", I cannot find the user you want to unban.\0";
			return false;
		}
	}
