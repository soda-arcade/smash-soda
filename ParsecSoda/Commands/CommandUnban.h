...
public:
	...
	bool run() override
	{
		...
		{
			_replyMessage = Config::cfg.chatbotName + "Usage: !unban <username>\nExample: !unban melon\0";
			return false;
		}
		...
		if (found)
		{
			...
			reply
				<< Config::cfg.chatbotName << _sender.name << " has revoked a ban:\n"
		...
		{
			_replyMessage = std::string() + Config::cfg.chatbotName + _sender.name + ", I cannot find the user you want to unban.\0";
			return false;
		}
	}
