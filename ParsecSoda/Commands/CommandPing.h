...
public:
	...
	bool run() override {
		...
		case SEARCH_USER_RESULT::NOT_FOUND:
			...
			_replyMessage = std::string() + "[MasterHand] | " + _sender.name + " just missed the guest they tried to ping check.\0";
		...
		case SEARCH_USER_RESULT::FOUND:
			...
			_replyMessage = std::string() + "[MasterHand] | " + _targetGuest.name + ": [" + ping.str() + "ms]\0";

			break;

		case SEARCH_USER_RESULT::FAILED:
		default:
			_replyMessage = "[MasterHand] | Usage: !ping <username>\nExample: !ping bigboi83\0";
			break;
		}
