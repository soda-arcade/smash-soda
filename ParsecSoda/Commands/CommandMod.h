...
public:
	...
	bool run() override
	{
		...
		case SEARCH_USER_HISTORY_RESULT::NOT_FOUND:
			_replyMessage = string() + "[ChatBot] | " + _sender.name + ", I cannot find the user you want to make a mod.\0";
			break;
		...
		default:
			_replyMessage = "[ChatBot] | Usage: !mod <username>\nExample: !mod melon\0";
			break;
		}
...
private:
	...
	bool handleGuest(GuestData target, bool isOnline, uint32_t guestID = -1)
	{
		...
		if (_sender.userID == target.userID)
			_replyMessage = string() + "[ChatBot] | You don't need to be a mod...you're the boss, " + _sender.name + "!\0";
		else
		{
			_replyMessage = string() + "[ChatBot] | " + target.name + " was made a mod by " + _sender.name + "!\0";
